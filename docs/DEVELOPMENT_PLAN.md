...
## Fase 17 � Seguran�a de conta e paridade MAUI

**Objetivo:** fechar gaps de seguran�a de acesso (recupera��o de senha, troca de senha, reset de 2FA) e garantir paridade funcional do app MAUI com o portal Web.

1. **[#168]** Reset do 2FA via Painel Admin � endpoint POST /admin/usuarios/{id}/reset-2fa + bot�o no painel Admin para operador redefinir o TOTP de usu�rio bloqueado.
2. **[#169]** Alterar pr�pria senha � endpoint PUT /auth/minha-senha + se��o em /minha-conta (Web) e MinhaContaPage (MAUI). Invalida refresh tokens ativos.
3. **[#170]** Recupera��o de senha via e-mail � endpoints POST /auth/esqueci-senha + POST /auth/redefinir-senha, migration senha_redefinicao, template de e-mail, p�ginas no Web e link no MAUI.
4. **[#171]** P�gina "Minha Empresa" no MAUI � MinhaEmpresaPage.xaml + MinhaEmpresaViewModel.cs com edi��o de dados da empresa e encerramento de conta, paridade com /minha-empresa do Web.
