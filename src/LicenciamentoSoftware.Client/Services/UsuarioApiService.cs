...
/// <summary>Proxy HTTP para os endpoints de Usuários da API.</summary>
public sealed class UsuarioApiService(HttpClient http)
{
        ...
        CancellationToken ct = default)
    {
        ...
        if (nome is not null) query["nome"] = nome;
        ...
        return await http.GetFromJsonAsync<PagedResult<UsuarioResult>>(
            $"usuarios?{query}", ct);
    }

    public async Task<UsuarioResult?> BuscarPorIdAsync(Guid id, CancellationToken ct = default)
        => await http.GetFromJsonAsync<UsuarioResult>($"usuarios/{id}", ct);

    public async Task<(bool Sucesso, UsuarioResult? Result, string? Erro, IReadOnlyList<string>? Erros)> CriarAsync(
        CriarUsuarioRequest request,
        CancellationToken ct = default)
    {
        var response = await http.PostAsJsonAsync("usuarios", request, ct);
        return await ParseWriteResponseAsync<UsuarioResult>(response, ct);
    }

    public async Task<(bool Sucesso, UsuarioResult? Result, string? Erro, IReadOnlyList<string>? Erros)> AtualizarAsync(
        Guid id,
        AtualizarUsuarioRequest request,
        CancellationToken ct = default)
    {
        var response = await http.PutAsJsonAsync($"usuarios/{id}", request, ct);
        return await ParseWriteResponseAsync<UsuarioResult>(response, ct);
    }

    public async Task<(bool Sucesso, string? Erro)> DesativarAsync(Guid id, CancellationToken ct = default)
    {
        var response = await http.DeleteAsync($"usuarios/{id}", ct);
